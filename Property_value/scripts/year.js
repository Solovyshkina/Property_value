document.addEventListener("DOMContentLoaded", function() {
    ...
    input.addEventListener("input", function() {
        if (!isNaN(parseFloat(this.value))) {
            var val = Math.min(2024, Math.max(1700, parseFloat(this.value))); // Ограничиваем значение в пределах допустимого диапазона
            ...
            slider.value = 1700; // Возвращаем значение по умолчанию
            sliderValue.textContent = 1700; // Обновляем текст над ползунком
            selectedYear = input.value; // Обновляем значение selectedYear
            updateSliderValuePosition(); // Обновляем позицию значения над ползунком на линии
            console.log(selectedYear); // Выводим значение selectedYear в консоль
        }
